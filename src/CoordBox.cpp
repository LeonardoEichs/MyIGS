...
/* ============================================================================================= */
{
    ...
    std::stringstream stream_x2, stream_y2, stream_z2;
    std::stringstream stream_x3, stream_y3, stream_z3;

    stream_x2 << m_entries[p3_index - 5]->get_text().raw();
    stream_y2 << m_entries[p3_index - 4]->get_text().raw();
    stream_z2 << m_entries[p3_index - 3]->get_text().raw();
    stream_x3 << m_entries[p3_index - 2]->get_text().raw();
    stream_y3 << m_entries[p3_index - 1]->get_text().raw();
    stream_z3 << m_entries[p3_index - 0]->get_text().raw();
    stream_x2 >> m_x2;
    stream_y2 >> m_y2;
    stream_z2 >> m_z2;
    stream_x3 >> m_x3;
    ...
    m_entries[p3_index + 3]->set_text(std::to_string(m_z3));
    ...
    if (m_z3 - m_z2 == 0.0) {
        m_entries[p3_index + 5]->set_text(std::to_string(m_z3));
        m_entries[p3_index + 5]->set_sensitive(false);
        enable_out_event = false;
    }
    if (enable_out_event) {
...
/* ============================================================================================= */
{
    ...
    for (unsigned i = 0; i < m_entries.size(); i += 2) {
        ...
        if (stream_x.str().size() != 0 && stream_y.str().size() != 0 && stream_z.str().size() != 0) {
            stream_x >> x;
            stream_y >> y;
            stream_z >> z;
